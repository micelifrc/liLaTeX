...
\codice{\bs auto[col=blue]\{0,0\};\\
\bs auto[col=red,mirror]\{4,0\};\\
\bs circCR[col=Green]\{8,1\}\{1\};\\
\bs circCR[col=orange,fill]\{11,1\}\{1\};}
...
\bs foreach \bs y in \{-1,1,2,...,8\} \bs asseybar[mirror]\{0.5*\bs y\}\{\$\bs y\$\};}
...
\codice{\bs sistrif[no label]\{0,0\}\{0\};\\
\bs sistrif[col=teal,lbl x=\$t\$]\{4,0\}\{30\};\\
\bs sistrif[col=gray!70,lbl x=\$\bs ra\$,mirror]\{8,0\}\{15\};}
...
\bs angolo[lbl=\$\bs beta\$,col=Green,fill,mark=||]\{A\}\{B\}\{C\};\\
\bs segmento[col=red,lbl=\$\bs ra\$]\{B\}\{A\};\\
...
\bs angolo[col=Green,fill,lbl=\$\bs ang\string^\bs circ\$,lbl dist=0.4]\{E\}\{O\}\{NE\};\\
\bs coordinate (C) at (\$(NE)+(45:0.3)\$);\\
\bs draw (\$(C)+(0.9,0)\$)\dash++(0,-1.5);\\
\bs draw (\$(C)+(\bs ang+90:0.9)\$)\dash++(\bs ang:-2.5)node(top)\{\};\\
\bs cassa[col=red,lbl=\$\bs mathbf\{m\_2\}\$]\{\$(C)+(0.9-0.4,-1.5)\$\}\{0.8\}\{-1\}\{0\};\\
\bs proj\{H\}\{top\}\{O\}\{NE\};\\
\bs cassa[col=blue,lbl=\$\bs mathbf\{m\_1\}\$]\{H\}\{-1.8\}\{1.2\}\{\bs ang\};\\
\bs carrucola\{C\}\{1\};\\
\bs graffa[lbl=\{\$2\{,\}4\bs,\bs SI\{m\}\$\}]\{O\}\{E\};\\
\bs coordinate (A) at (\bs ang:2);\\
\bs auto[col=red,ang=\bs ang]\{\bs ang:2\};}
...
\item L'opzione \keyop{mirror}, se specificata, specchi l'immagine.
\end{itemize}

Vediamo un semplice esempio:

\codice{\bs grigliaO[step=0.5]\{6\}\{1\};\\
\bs casetta\{2,0\};\\
\bs uomo[col=blue]\{4,0\};\\
\bs donna[mirror,col=red]\{5,0\};}
...
\bs moto[col=blue,mirror]\{12,0\};\\
\bs bici[col=Green,side=<-,mirror]\{14,0\};}
