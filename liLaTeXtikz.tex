...
\newcommand{\getdistanza}[2]{
	\coordinate(getdistanzafrom)at(#1);
	\coordinate(getdistanzato)at(#2);
	\tkzCalcLength(getdistanzafrom,getdistanzato)
...
\newcommand{\stylelbl}[2][]{
	...
	\PRIVstylelbl[
		PRIVlbl lbl style=\kv{lbl style},
		PRIVlbl lbl=\kv{lbl},
...
\newcommand{\PRIVstylelbl}[2][]{
	\tikzset{keyfld/.cd,
		PRIVlbl lbl style/.initial, % se specificato usiamo questo stile. Le opzioni sono rm, bf, cal
		PRIVlbl lbl/.initial,
	...
	\ifkeyempty{PRIVlbl lbl}{\coordinate (\kv{PRIVlbl lbl name}) at (#2);}{
		\ifkeyempty{PRIVlbl lbl style}{ \node[\kv{PRIVlbl col lbl}] (\kv{PRIVlbl lbl name}) at (#2) {\kv{PRIVlbl lbl size}\kv{PRIVlbl lbl}}; }{}
		\ifkeyequal{PRIVlbl lbl style}{rm}{ \node[\kv{PRIVlbl col lbl}] (\kv{PRIVlbl lbl name}) at (#2) {\kv{PRIVlbl lbl size}$\mathrm{\kv{PRIVlbl lbl}}$}; }{}
		\ifkeyequal{PRIVlbl lbl style}{PRIVlbl bf}{ \node[\kv{PRIVlbl col lbl}] (\kv{PRIVlbl lbl name}) at (#2) {\kv{PRIVlbl lbl size}$\mathbf{\kv{PRIVlbl lbl}}$}; }{}
		\ifkeyequal{PRIVlbl lbl style}{PRIVlbl cal}{ \node[\kv{PRIVlbl col lbl}] (\kv{PRIVlbl lbl name}) at (#2) {\kv{PRIVlbl lbl size}$\mathcal{\kv{PRIVlbl lbl}}$}; }{}
	}
}
...
\newcommand{\punto}[2][]{
	\tikzset{keyfld/.cd,
		...
		lbl/.initial,
		lbl style/.initial, % può essere lasciato vuoto o può essere rm, bf, cal
		lbl name/.initial,
		lbl ang/.initial = -90,
...
% \grigliaZ[keys]{east}{north}. Ha le stesse keys di \griglia
...
\newcommand{\assex}[3][]{
	...
	\PRIVstylelbl[
		...
		PRIVlbl lbl col=\kv{lbl col},
		PRIVlbl col lbl=\kv{col lbl},
	]{$(assexpunta)+(\kv{ang shift}-90+\assexflipper:\kv{lbl dist})$};
}
...
\newcommand{\assey}[3][]{
	...
	\PRIVstylelbl[
		...
		PRIVlbl lbl col=\kv{lbl col},
		PRIVlbl col lbl=\kv{col lbl},
	]{$(asseypunta)+(\kv{ang shift}+180+\asseyflipper:\kv{lbl dist})$};
}
...
\newcommand{\piedebis}[5][piedebisTemp]{
	\coordinate(piedebisA)at(#3);
	\coordinate(piedebisB)at(#4);
	\coordinate(piedebisC)at(#5);
...
\newcommand{\triangolobase}[4][]{
	\tikzset{keyfld/.cd,
		...
		col lati/.initial,
		col angoli/.initial,
		fill angoli/.initial = false,
		width/.initial, % può essere thin, thick, ...
		pattern/.initial, % può essere solid, dashed, dotted, ...
		lbl vertici/.initial = true,
		lbl lati/.initial = true,
		lbl angoli/.initial = true,
	...
	\copykeyvaluesifempty
		...
		{col lati}{col}
		{col angoli}{col}
	\ifkeyequal{lbl angoli}{true}{
		\PRIVangolo[PRIV col=\kv{col angoli},PRIV fill=\kv{fill angoli},PRIV lbl=$\alpha$]{#3}{#2}{#4};
		\PRIVangolo[PRIV col=\kv{col angoli},PRIV fill=\kv{fill angoli},PRIV lbl=$\beta$]{#4}{#3}{#2};
		\PRIVangolo[PRIV col=\kv{col angoli},PRIV fill=\kv{fill angoli},PRIV lbl=$\gamma$]{#2}{#4}{#3};
	}{
		\PRIVangolo[PRIV col=\kv{col angoli},PRIV fill=\kv{fill angoli}]{#3}{#2}{#4};
		\PRIVangolo[PRIV col=\kv{col angoli},PRIV fill=\kv{fill angoli}]{#4}{#3}{#2};
		\PRIVangolo[PRIV col=\kv{col angoli},PRIV fill=\kv{fill angoli}]{#2}{#4}{#3};
	}
	\ifkeyequal{lbl lati}{true}{
		\PRIVsegmento[PRIV col=\kv{col lati},PRIV lbl=a,PRIV lbl style=rm]{#3}{#4};
		\PRIVsegmento[PRIV col=\kv{col lati},PRIV lbl=b,PRIV lbl style=rm]{#4}{#2};
		\PRIVsegmento[PRIV col=\kv{col lati},PRIV lbl=c,PRIV lbl style=rm]{#2}{#3};
	}{
		\PRIVsegmento[PRIV col=\kv{col lati}]{#3}{#4};
		\PRIVsegmento[PRIV col=\kv{col lati}]{#4}{#2};
		\PRIVsegmento[PRIV col=\kv{col lati}]{#2}{#3};
	}
	\ifkeyequal{lbl vertici}{true}{
...
\newcommand{\PRIVsegmento}[3][]{
	\tikzset{keyfld/.cd,
		...
		PRIV lbl/.initial,
		PRIV lbl style/.initial, % può essere vuoto, oppure può essere rm,bf,cal
	...
	\ifkeyempty{PRIV lbl}{}{\setkeyvalueifempty{PRIV lbl name}{segmentoLabelPos};}
	...
	\PRIVstylelbl[
		PRIVlbl lbl style=\kv{PRIV lbl style},
...
\newcommand{\PRIVangolo}[4][]{
	\tikzset{keyfld/.cd,
		...
		PRIV opacity/.initial = 0.3, % un numero fra 0 e 1 ch indica l'intensità della colorazione
		PRIV lbl/.initial,
		PRIV lbl dist/.initial = 0.2, % la distanza da aggiungere al raggio
		PRIV lbl size/.initial,
		PRIV lbl col/.initial,
	...
	\ifkeyequal{PRIV fill}{true}{\tkzFillAngle[size=\kv{PRIV r},opacity=\kv{PRIV opacity},fill=\kv{PRIV col fill}](angoloPuntoA,angoloPuntoB,angoloPuntoC)}{}
	...
	\ifkeyempty{PRIV lbl}{}{\tkzLabelAngle[pos=\kv{PRIV r}+\kv{PRIV lbl dist},\kv{PRIV lbl col}](angoloPuntoA,angoloPuntoB,angoloPuntoC){\kv{PRIV lbl size}\kv{PRIV lbl}}}
}
...
\newcommand{\retta}[3][]{
	\tikzset{keyfld/.cd,
		...
		lbl/.initial,
		lbl style/.initial, % può essere lasciato vuoto oppure può essere rm, bf, cal
		lbl name/.initial, % il nome da dare alla posizione del label
		lbl dist/.initial = 0.2, % la distanza da aggiungere al raggio
...
\newcommand{\uomo}[2][]{
	...
	\ifkeyequal{side}{->}{\def\uomoflipper{}}{\def\uomoflipper{180-}}
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.9*\kv{h})$)circle(0.1*\kv{h}); % testa
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.8*\kv{h})$)--++(0,-0.1*\kv{h}); % collo
	\draw[\kv{col},ultra thick]($(#2)+(0,0.7*\kv{h})$)--++(0,-0.3*\kv{h}); % corpo
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\kv{h})$)--++(\uomoflipper\kv{ang braccio 1}:\kv{lung braccio}*\kv{h})node(\kv{mano 1}){}; % braccio 1
	\ifkeyempty{ang braccio 2}{}{\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\kv{h})$)--++(\uomoflipper\kv{ang braccio 2}:\kv{lung braccio}*\kv{h})node(\kv{mano 2}){};} % braccio 2
	\intrette{\kv{piede 1}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\kv{h})$}{$(#2)+(0,0.4*\kv{h})+(\uomoflipper\kv{ang gamba 1}:1)$}; % la posizione del primo piede
	\intrette{\kv{piede 2}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\kv{h})$}{$(#2)+(0,0.4*\kv{h})+(\uomoflipper\kv{ang gamba 2}:1)$}; % la posizione del secondo piede
	\draw[\kv{col},\kv{width}](\kv{piede 1})--($(#2)+(0,0.4*\kv{h})$)--(\kv{piede 2}); % le gambe
}
...
\newcommand{\donna}[2][]{
	...
	\ifkeyempty{ang braccio}{}{\copykeyvalue{ang braccio 1}{ang braccio};}
	\ifkeyempty{ang gamba}{}{\copykeyvalue{ang gamba 1}{ang gamba};}
	\ifkeyequal{side}{->}{\def\donnaflipper{}}{\def\donnaflipper{180-}}
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.9*\kv{h})$)circle(0.1*\kv{h}); % testa
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.8*\kv{h})$)--++(0,-0.1*\kv{h}); % collo
	\draw[\kv{col},ultra thick]($(#2)+(0,0.7*\kv{h})$)--++(0,-0.3*\kv{h}); % corpo
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.4*\kv{h})$)--++(-0.2*\kv{h},-0.2*\kv{h})--++(+0.4*\kv{h},0)--++(-0.2*\kv{h},+0.2*\kv{h}); % gonna
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\kv{h})$)--++(\donnaflipper\kv{ang braccio 1}:\kv{lung braccio}*\kv{h})node(\kv{mano 1}){}; % braccio 1
	\ifkeyempty{ang braccio 2}{}{\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\kv{h})$)--++(\donnaflipper\kv{ang braccio 2}:\kv{lung braccio}*\kv{h})node(\kv{mano 2}){};} % braccio 2
	\intrette{\kv{piede 1}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\kv{h})$}{$(#2)+(0,0.4*\kv{h})+(\donnaflipper\kv{ang gamba 1}:1)$}; % la posizione del primo piede
	\intrette{\kv{piede 2}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\kv{h})$}{$(#2)+(0,0.4*\kv{h})+(\donnaflipper\kv{ang gamba 2}:1)$}; % la posizione del secondo piede
	\medio{GambaDxGonna}{\kv{piede 1}}{$(#2)+(0,0.4*\kv{h})$};
	\medio{GambaSxGonna}{\kv{piede 2}}{$(#2)+(0,0.4*\kv{h})$};
...
\newcommand{\auto}[2][]{
	...
	\copykeyvaluesifempty
		...
		{y}{h}
		{s}{scale}
	\ifkeyequal{side}{->}{\def\autoflipper{+1}}{\def\autoflipper{-1}}
	\tikzdef{\autosizex}{\autoflipper*\kv{s}*\kv{x}};
	\tikzdef{\autosizey}{\kv{s}*\kv{y}};
	% ruote
	\coordinate(autocentroruotaposteriore)at($(#2)+(-0.3*\autosizex,0.1*\autosizey)$);
	\coordinate(autocentroruotaanteriore)at($(#2)+(+0.15*\autosizex,0.1*\autosizey)$);
	\draw[\kv{col},\kv{width}](autocentroruotaposteriore)circle(0.1*\autosizey);
	\draw[\kv{col},\kv{width}](autocentroruotaanteriore)circle(0.1*\autosizey);
	\coordinate(autobottomfront)at($(#2)+(0.4*\autosizex,0.12*\autosizey)$);
	\coordinate(autobottomback)at($(#2)+(-0.5*\autosizex,0.12*\autosizey)$);
	\interLC{autoruotaanteriorefront}{autoruotaanterioreback}{autobottomfront}{autobottomback}{autocentroruotaanteriore}{0.12*\autosizey};
	\interLC{autoruotaposteriorefront}{autoruotaposterioreback}{autobottomfront}{autobottomback}{autocentroruotaposteriore}{0.12*\autosizey};
	\ifkeyequal{side}{->}{
	...
	%carrozzeria
	\draw[\kv{col},\kv{width},rounded corners=\kv{s}*0.5mm] (autoruotaposteriorefront)--(autoruotaanterioreback) (autoruotaanteriorefront)--(autobottomfront)--($(#2)+(0.5*\autosizex,0.25*\autosizey)$)--++(0,0.1*\autosizey)--++(-0.2*\autosizex,0.15*\autosizey)--++(0,0.5*\autosizey)--($(#2)+(-0.5*\autosizex,\autosizey)$)--(autobottomback)--(autoruotaposterioreback);
}
...
\newcommand{\camion}[2][]{
	...
	\copykeyvaluesifempty
		...
		{y}{h}
		{s}{scale}
	\ifkeyequal{side}{->}{\def\camionflipper{+1}}{\def\camionflipper{-1}}
	\tikzdef{\camionsizex}{\camionflipper*\kv{s}*\kv{x}};
	\tikzdef{\camionsizey}{\kv{s}*\kv{y}};
	% ruote
	\coordinate(camioncentroruotaposteriore)at($(#2)+(-0.4*\camionsizex,0.1*\camionsizey)$);
	\coordinate(camioncentroruotaanteriore)at($(#2)+(+0.35*\camionsizex,0.1*\camionsizey)$);
	\draw[\kv{col},\kv{width}](camioncentroruotaposteriore)circle(0.1*\camionsizey);
	\draw[\kv{col},\kv{width}](camioncentroruotaanteriore)circle(0.1*\camionsizey);
	\coordinate(camionbottomfront)at($(#2)+(+0.5*\camionsizex,0.12*\camionsizey)$);
	\coordinate(camionbottomback)at($(#2)+(-0.5*\camionsizex,0.12*\camionsizey)$);
	\interLC{camionruotaanteriorefront}{camionruotaanterioreback}{camionbottomfront}{camionbottomback}{camioncentroruotaanteriore}{0.12*\camionsizey};
	\interLC{camionruotaposteriorefront}{camionruotaposterioreback}{camionbottomfront}{camionbottomback}{camioncentroruotaposteriore}{0.12*\camionsizey};
	\ifkeyequal{side}{->}{
	...
	%carrozzeria
	\draw[\kv{col},\kv{width}] (camionruotaposteriorefront)--(camionruotaanterioreback) (camionruotaanteriorefront)--(camionbottomfront)--($(#2)+(0.5*\camionsizex,0.2*\camionsizey)$)--++(-\camionsizex/60,0) (camionruotaposterioreback)--(camionbottomback)--++(0,0.3*\camionsizey)--++(0.75*\camionsizex,0)--++(0,0.7*\camionsizey)--++(-0.72*\camionsizex,0)--++(0,-0.7*\camionsizey);
	\draw[\kv{col},\kv{width},rounded corners=\kv{s}*0.5mm] ($(#2)+(0.5*\camionsizex-\camionsizex/60,0.2*\camionsizey)$)--++(0,0.2*\camionsizey)--++(-\camionsizex/20,0)--++(0,0.4*\camionsizey)--++(-0.15*\camionsizex-\camionsizex/30,0);
}