...
\newcommand{\uomo}[2][]{
	...
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\kv{h})$)--++(\kv{ang braccio 1}:\kv{lung braccio}*\kv{h})node(\kv{mano 1}){}; % braccio 1
	\ifkeyempty{ang braccio 2}{}{\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\kv{h})$)--++(\kv{ang braccio 2}:\kv{lung braccio}*\kv{h})node(\kv{mano 2}){};} % braccio 2
	\intrette{\kv{piede 1}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\kv{h})$}{$(#2)+(0,0.4*\kv{h})+(\kv{ang gamba 1}:1)$}; % la posizione del primo piede
	\intrette{\kv{piede 2}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\kv{h})$}{$(#2)+(0,0.4*\kv{h})+(\kv{ang gamba 2}:1)$}; % la posizione del secondo piede
