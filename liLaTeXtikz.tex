...
\newcommand*{\setkeyfld}[1]{
	...
	\tikzset{keyfld/.cd,#1}%
	% definiamo un soprannome \kv per il path alle keys. In questo modo, per riferirci al VALORE di una chiave chiamata K dovremo scrivere \kv{K}, anziché la più lunga /tikz/keyfld/K. Quindi K indica la chiave, mentre \kv{K} indica il suo valore. \kp{K} indica invece l'indirizzo (il path) delle key K
...
\newcommand*{\angoloretto}[4][]{
	\tikzset{keyfld/.cd,
		col/.initial,
		col draw/.initial, % il colore con cui si disegna l'angolo
		s/.initial = 0.3, % il lato del segmento del quadratino
		r/.initial, % uguale a s
		width/.initial, % può essere thin, thick, ...
		pattern/.initial, % può essere solid, dashed, dotted, ...
		arrow/.initial, % può essere ->, <-, <-> (o lasciata vuota)
		fill/.initial = false, fill/.default = true, % può essere true o false
	...
	\verso[\kv{r}]{angolorettoPuntoA}{angolorettoPuntoB}{#2};
	\verso[\kv{r}]{angolorettoPuntoC}{angolorettoPuntoB}{#4};
	\sumvett{angolorettoPuntoD}{angolorettoPuntoB}{angolorettoPuntoA}{angolorettoPuntoC};
	\ifkeyequal{fill}{true}
		{\fill[\kv{col fill}] (angolorettoPuntoB) rectangle (angolorettoPuntoD);}{}
	\draw[\kv{col draw},\kv{width},\kv{pattern},\kv{arrow}] (angolorettoPuntoA) -- (angolorettoPuntoD) -- (angolorettoPuntoC);
}
...
% \parabola[opzioni]{y_V}{A}{B} disegna un arco di parabola di estremi A e B e il cui vertice, compreso fra P1 e P2, deve avere ordinata y_V
\newcommand*{\parabolayVPP}[4][]{
	\tikzdef\parabolayVPPyV{#2}
	\extractcoordpair{\parabolayVPPxA}{\parabolayVPPyA}{#3}
	\extractcoordpair{\parabolayVPPxB}{\parabolayVPPyB}{#4}
	\tikzdef\parabolayVPPsqrtDyA{sqrt(abs(\parabolayVPPyV-\parabolayVPPyA))}
	\tikzdef\parabolayVPPsqrtDyB{sqrt(abs(\parabolayVPPyV-\parabolayVPPyB))}
	\tikzdef\parabolayVPPratio{\parabolayVPPsqrtDyA/(\parabolayVPPsqrtDyA+\parabolayVPPsqrtDyB)}
	\medio[\parabolayVPPratio]{parabolayVPPmedio}{#3}{#4};
	\coordinate (parabolayVPPertice) at (parabolayVPPmedio|-0,\parabolayVPPyV);
	\parabolaVPPy[#1]{parabolayVPPertice}{#3}{#4};
}
...
\newcommand*{\extractcoordpair}[4][,]{
	...
	\PRIVextractcoordpairIterFist#4\extractcoordpairseparator
	\tikzdef#2{\extractcoordpairx}
	\tikzdef#3{\extractcoordpairy}
}
\newcommand*{\PRIVextractcoordpairIterFist}[1]{
	\if\extractcoordpairseparator#1 
		\expandafter\PRIVextractcoordpairIterSecond
	\else 
		\estendistr\extractcoordpairx{#1}
		\expandafter\PRIVextractcoordpairIterFist
	\fi
}
\newcommand*{\PRIVextractcoordpairIterSecond}[1]{
...
% \tavolo[keys]{centro base}{lunghezza x}{lunghezza y} disegna un tavolo
\newcommand*{\tavolo}[4][]{
	\tikzset{keyfld/.cd,
		col/.initial = Brown,
		col fill/.initial = Brown!40,
		col vetro/.initial = cyan,
		no fill/.initial = false, no fill/.default = true, % se specificata evita di fillare l'immagine
		width/.initial, % può essere thin, thick, ...
		pattern/.initial, % può essere solid, dashed, dotted, ...
		h top/.initial, % l'altezza della parte superiore del tavolo
		w gambe/.initial, % la larghezza delle gambe inferiore del tavolo
	} \setkeyfld{#1};
	\tikzdef\xtavolo{#3}
	\tikzdef\ytavolo{#4}
	\tikzdef\htoptavolo{min(\ytavolo/5,0.5)}
	\setkeyvalueifempty{h top}{\htoptavolo}
	\tikzdef\wgambetavolo{min(\xtavolo/10,0.4)}
	\setkeyvalueifempty{w gambe}{\wgambetavolo}
	\tikzdef\dxgambetavolo{min((\xtavolo-2*(\kv{w gambe}+0.1))/5,0.3)}
	\ifkeyequal{no fill}{false}{
		\fill[\kv{col fill}] ($(#2)+(\xtavolo/2-0.1,\ytavolo-0.2*\kv{h top})$) --++ (0,-0.8*\kv{h top}) --++ (-\dxgambetavolo,0) --++ (-0.15*\kv{w gambe},-\ytavolo+\kv{h top}) --++ (-0.7*\kv{w gambe},0) --++ (-0.15*\kv{w gambe},+\ytavolo-\kv{h top}) --++ (-\xtavolo+0.2+2*\dxgambetavolo+2*\kv{w gambe},0) --++ (-0.15*\kv{w gambe},-\ytavolo+\kv{h top}) --++ (-0.7*\kv{w gambe},0) --++ (-0.15*\kv{w gambe},+\ytavolo-\kv{h top}) --++ (-\dxgambetavolo,0) --++ (0,+0.8*\kv{h top}); % la parte di legno
		\fill[\kv{col vetro}!40] ($(#2)+(\xtavolo/2,\ytavolo)$) rectangle++ (-\xtavolo,-0.2*\kv{h top}); % la parte di vetro
	}{}
	\draw[\kv{width},\kv{pattern},\kv{col}] ($(#2)+(\xtavolo/2-0.1,\ytavolo-0.2*\kv{h top})$) --++ (0,-0.8*\kv{h top}) --++ (-\dxgambetavolo,0) --++ (-0.15*\kv{w gambe},-\ytavolo+\kv{h top}) --++ (-0.7*\kv{w gambe},0) --++ (-0.15*\kv{w gambe},+\ytavolo-\kv{h top}) --++ (-\xtavolo+0.2+2*\dxgambetavolo+2*\kv{w gambe},0) --++ (-0.15*\kv{w gambe},-\ytavolo+\kv{h top}) --++ (-0.7*\kv{w gambe},0) --++ (-0.15*\kv{w gambe},+\ytavolo-\kv{h top}) --++ (-\dxgambetavolo,0) --++ (0,+0.8*\kv{h top}); % la parte di legno
	\draw[\kv{width},\kv{pattern},\kv{col vetro}] ($(#2)+(\xtavolo/2,\ytavolo)$) rectangle++ (-\xtavolo,-0.2*\kv{h top}); % la parte di vetro
}

% \mezzotavolo[keys]{punto base tagliato}{lunghezza x}{lunghezza y} disegna un tavolo
\newcommand*{\mezzotavolo}[4][]{
	\tikzset{keyfld/.cd,
		col/.initial = Brown,
		col fill/.initial = Brown!40,
		col vetro/.initial = cyan,
		...
		h top/.initial, % l'altezza della parte superiore del tavolo
		w gambe/.initial, % la larghezza delle gambe inferiore del tavolo
		side/.initial = ->, % da che lato orientare il tavolo
		mirror/.initial = false, mirror/.default = true, % se specificata flippa l'immagine
	} \setkeyfld{#1};
	\ifkeyequal{mirror}{true}{
		\ifkeyequal{side}{->}{\setkeyvalue{side}{<-}}{\setkeyvalue{side}{->}}
	}{}
	\ifkeyequal{side}{->}{\tikzdef\tavolomirror{+1}}{\tikzdef\tavolomirror{-1}}
	\tikzdef\xtavolo{#3}
	\tikzdef\ytavolo{#4}
	\tikzdef\htoptavolo{min(\ytavolo/5,0.5)}
	\setkeyvalueifempty{h top}{\htoptavolo}
	\tikzdef\wgambetavolo{min(\xtavolo/5,0.4)}
	\setkeyvalueifempty{w gambe}{\wgambetavolo}
	\tikzdef\dxgambetavolo{min((\xtavolo-(\kv{w gambe}+0.1))/2.5,0.3)}
	\ifkeyequal{no fill}{false}{
		\fill[\kv{col fill}] ($(#2)+(\tavolomirror*\xtavolo-\tavolomirror*0.1,\ytavolo-0.2*\kv{h top})$) --++ (0,-0.8*\kv{h top}) --++ (-\tavolomirror*\dxgambetavolo,0) --++ (-\tavolomirror*0.15*\kv{w gambe},-\ytavolo+\kv{h top}) --++ (-\tavolomirror*0.7*\kv{w gambe},0) --++ (-\tavolomirror*0.15*\kv{w gambe},+\ytavolo-\kv{h top}) --++ (-\tavolomirror*\xtavolo+\tavolomirror*0.1+\tavolomirror*\dxgambetavolo+\tavolomirror*\kv{w gambe},0) --++ (0,+0.8*\kv{h top}); % la parte di legno
		\fill[\kv{col vetro}!40] ($(#2)+(0,\ytavolo)$) --++ (\tavolomirror*\xtavolo,0) --++ (0,-0.2*\kv{h top}) --++ (-\tavolomirror*\xtavolo,0); % la parte di vetro
	}{}
	\draw[\kv{width},\kv{pattern},\kv{col}] ($(#2)+(\tavolomirror*\xtavolo-\tavolomirror*0.1,\ytavolo-0.2*\kv{h top})$) --++ (0,-0.8*\kv{h top}) --++ (-\tavolomirror*\dxgambetavolo,0) --++ (-\tavolomirror*0.15*\kv{w gambe},-\ytavolo+\kv{h top}) --++ (-\tavolomirror*0.7*\kv{w gambe},0) --++ (-\tavolomirror*0.15*\kv{w gambe},+\ytavolo-\kv{h top}) --++ (-\tavolomirror*\xtavolo+\tavolomirror*0.1+\tavolomirror*\dxgambetavolo+\tavolomirror*\kv{w gambe},0) ++ (0,+0.8*\kv{h top}); % la parte di legno
	\draw[\kv{width},\kv{pattern},\kv{col vetro}] ($(#2)+(0,\ytavolo)$) --++ (\tavolomirror*\xtavolo,0) --++ (0,-0.2*\kv{h top}) --++ (-\tavolomirror*\xtavolo,0); % la parte di vetro
}
...
\newcommand*{\auto}[2][]{
	...
	\ifkeyequal{side}{->}{\def\automirrorper{+1}}{\def\automirrorper{-1}}
	...
	\draw[\kv{col},\kv{width},rounded corners=\kv{s}*0.5mm] (autoruotaposteriorefront) -- (autoruotaanterioreback)  (autoruotaanteriorefront) -- (autobottomfront) -- ($(#2)+(15*\autosizex,5*\autosizey)$) --++ (0,2*\autosizey) --++ (-6*\autosizex,3*\autosizey) --++ (0,10*\autosizey) -- ($(#2)+(-15*\autosizex,20*\autosizey)$) -- (autobottomback) -- (autoruotaposterioreback);
		...
		\coordinate (centrorotazione) at (#2);
		\begin{rotazione}[centrorotazione]{\kv{ang}} \auto[#1,ang=]{centrorotazione} \end{rotazione}
	}
}
...
\newcommand*{\camion}[2][]{
	...
	\ifkeyequal{side}{->}{\def\camionmirrorper{+1}}{\def\camionmirrorper{-1}}
	...
	\draw[\kv{col},\kv{width},rounded corners=\kv{s}*0.5mm] ($(#2)+(29*\camionsizex,6*\camionsizey)$) --++ (0,6*\camionsizey) --++ (-3*\camionsizex,0) --++ (0,12*\camionsizey) --++ (-11*\camionsizex,0);
		...
		\coordinate (centrorotazione) at (#2);
		\begin{rotazione}[centrorotazione]{\kv{ang}} \camion[#1,ang=]{centrorotazione} \end{rotazione}
	}
}
...
\newcommand*{\moto}[2][]{
	...
	\ifkeyequal{side}{->}{\def\motomirrorper{+1}}{\def\motomirrorper{-1}}
	...
	\draw[\kv{col},\kv{width}](motocentrocasco) circle (2*\motosizey);
		...
		\coordinate (centrorotazione) at (#2);
		\begin{rotazione}[centrorotazione]{\kv{ang}} \moto[#1,ang=]{centrorotazione} \end{rotazione}
	}
}
...
\newcommand*{\bici}[2][]{
	...
	\ifkeyequal{side}{->}{\def\bicimirrorper{+1}}{\def\bicimirrorper{-1}}
	...
	\draw[\kv{col},\kv{width}] ($(bicicollo)+(0,2*\bicisizey)$) circle (2*\bicisizey);
		...
		\coordinate (centrorotazione) at (#2);
		\begin{rotazione}[centrorotazione]{\kv{ang}} \bici[#1,ang=]{centrorotazione} \end{rotazione}
	}
}