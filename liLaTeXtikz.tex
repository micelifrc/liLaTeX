...
% Attenzione! Se abbiamo qualcosa come \def\nome{$a$} la funzione \removedollar{$a$} funziona correttamente, ma la funzione \removedollar{\nome} non funziona (lavora sulla stringa \nome anziché sulla stringa $a$)
...
%TODO idealmente vorremmo creare una funzione come removedollarfromkey che funzioni con tutte le keys (senza bisogno di specificare il valore di #2)
...
\newcommand{\cassa}[5][]{
	...
	\ifkeyequal{fill}{true}{\fill[\kv{col fill}] (#2)--++(#5:#3)--++(#5+90:#4)--++(#5+180:#3)--++(#5+270:#4);}{}
...
\newcommand{\uomo}[2][]{
	...
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\uomoh)$)--++(\uomoflipper\kv{ang braccio 1}:\kv{lung braccio}*\uomoh)node(\kv{mano 1}){}; % braccio 1
	\ifkeyempty{ang braccio 2}{}{\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\uomoh)$)--++(\uomoflipper\kv{ang braccio 2}:\kv{lung braccio}*\uomoh)node(\kv{mano 2}){};} % braccio 2
	\intrette{\kv{piede 1}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\uomoh)$}{$(#2)+(0,0.4*\uomoh)+(\uomoflipper\kv{ang gamba 1}:1)$}; % la posizione del primo piede
	\intrette{\kv{piede 2}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\uomoh)$}{$(#2)+(0,0.4*\uomoh)+(\uomoflipper\kv{ang gamba 2}:1)$}; % la posizione del secondo piede
...
\newcommand{\donna}[2][]{
	...
	\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\donnah)$)--++(\donnaflipper\kv{ang braccio 1}:\kv{lung braccio}*\donnah)node(\kv{mano 1}){}; % braccio 1
	\ifkeyempty{ang braccio 2}{}{\draw[\kv{col},\kv{width}]($(#2)+(0,0.7*\donnah)$)--++(\donnaflipper\kv{ang braccio 2}:\kv{lung braccio}*\donnah)node(\kv{mano 2}){};} % braccio 2
	\intrette{\kv{piede 1}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\donnah)$}{$(#2)+(0,0.4*\donnah)+(\donnaflipper\kv{ang gamba 1}:1)$}; % la posizione del primo piede
	\intrette{\kv{piede 2}}{#2}{$(#2)+(1,0)$}{$(#2)+(0,0.4*\donnah)$}{$(#2)+(0,0.4*\donnah)+(\donnaflipper\kv{ang gamba 2}:1)$}; % la posizione del secondo piede
