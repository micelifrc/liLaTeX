...
% crea un \inputwithoutdollar uguale all'input #1 ma senza i $
% Attenzione! non funziona se l'input è fornito da una key (non so perché)
\newcommand*{\removedollar}[1]{{\catcode`\$=9\scantokens{\xdef\inputwihoutdollar{#1}}}}

% rimuove lo \ dall'input
\newcommand{\removeslash}[1]{{\catcode92=9 \endlinechar-1 \scantokens{#1}}}

% rimuove i $ dall'input
\newcommand{\removemathmode}[1]{{\catcode24=9 \endlinechar-1 \scantokens{#1}}}
...
\newcommand{\assixy}[5][]{
	...
	\copykeyvaluesifempty
		{lbl x style}{lbl style}
		{lbl y style}{lbl style}
	...
	\PRIVstylelbl[
		PRIVlbl lbl style=\kv{lbl x style},
	...
	\PRIVstylelbl[
		PRIVlbl lbl style=\kv{lbl y style},
...
\newcommand{\sistrif}[3][]{
	\tikzset{keyfld/.cd,
		...
		label/.initial = true, % rendi false se non vuoi il label
		lbl dist/.initial = 0.3,
		lbl x/.initial = $x$,
		lbl y/.initial = $y$,
		lbl col/.initial,
	...
	\ifkeyequal{label}{true}{
		\PRIVstylelbl[PRIVlbl lbl style=\kv{lbl x style},PRIVlbl lbl=\kv{lbl x},PRIVlbl col lbl=\kv{col lbl}]{$(sistrifLblXpunta)+(#3-90*\sistrifflipper:\kv{lbl dist})$};
		\PRIVstylelbl[PRIVlbl lbl style=\kv{lbl y style},PRIVlbl lbl=\kv{lbl y},PRIVlbl col lbl=\kv{col lbl}]{$(sistrifLblYpunta)+(#3+180:\kv{lbl dist})$};
	}{}
}
...
\newcommand{\segmento}[3][]{
	\tikzset{keyfld/.cd,
		...
		col mark/.initial, % il colore del segno
		width/.initial, % può essere thin, thick, ...
		pattern/.initial, % può essere solid, dashed, dotted, ...
...
\newcommand{\PRIVsegmento}[3][]{
	\tikzset{keyfld/.cd,
		...
		PRIV col mark/.initial, % il colore del segno
...
\newcommand{\carrucola}[3][]{
	\tikzset{keyfld/.cd,
		...
		col3/.initial, % il colore della parte interna
...
\newcommand{\auto}[2][]{
	...
	\tikzdef{\autosizex}{\autoflipper*\kv{s}*\kv{x}/30};
...
\newcommand{\camion}[2][]{
	...
	\tikzdef{\camionsizex}{\camionflipper*\kv{s}*\kv{x}/60};
...
\newcommand{\moto}[2][]{
	...
	\copykeyvaluesifempty
		...
		{y}{h}
		{s}{scale}
	\ifkeyequal{side}{->}{\def\motoflipper{+1}}{\def\motoflipper{-1}}
	\tikzdef{\motosizex}{\motoflipper*\kv{s}*\kv{x}/20};
	\tikzdef{\motosizey}{\kv{s}*\kv{y}/15};
	\coordinate(motocentroruotaposteriore)at($(#2)+(-10*\motosizex+\motoflipper*2*\motosizey,2*\motosizey)$);
	\coordinate(motocentroruotaanteriore)at($(#2)+(+10*\motosizex-\motoflipper*2*\motosizey,2*\motosizey)$);
	...
	\ifkeyequal{side}{->}
	{\coordinate(motofrontattachpoint)at($(#2)+(+10*\motosizex-\motoflipper*2*\motosizey,3*\motosizey)+(110:3*\motosizey)$);}
	{\coordinate(motofrontattachpoint)at($(#2)+(+10*\motosizex-\motoflipper*2*\motosizey,3*\motosizey)+(70:3*\motosizey)$);}
	\draw[\kv{col},\kv{width}](motocentroruotaanteriore)--(motofrontattachpoint);
	\draw[\kv{col},\kv{width}](motocentroruotaposteriore)--($(motocentroruotaanteriore)+(-\motoflipper*3*\motosizey,0)$)--++(0,\motosizey)arc(90+\motoflipper*90:90:3*\motosizey)--++(\motosizex,0)arc(90-\motoflipper*90:90:4*\motosizey)--++(0,-2*\motosizey)--++(-4*\motosizex,0)arc(90:90+\motoflipper*90:\motosizey cm and 2*\motosizey cm)--++(-5*\motosizex,0)--++(-\motosizex,\motosizey)--++(-4*\motosizex,0)--++(0,-\motosizey)arc(90:90-\motoflipper*90:5*\motosizey cm and 3*\motosizey cm)--++(0,-\motosizey);
	...
	\draw[\kv{col},\kv{width}]($(motocentrocasco)+(90-\motoflipper*60:2*\motosizey)$)--($(motocentrocasco)+(0,\motosizey)$)--($(motocentrocasco)+(0,-\motosizey)$)--($(motocentrocasco)+(-90+\motoflipper*60:2*\motosizey)$);
...
\newcommand{\bici}[2][]{
	...
	\copykeyvaluesifempty
		...
		{y}{h}
		{s}{scale}
	\ifkeyequal{side}{->}{\def\biciflipper{+1}}{\def\biciflipper{-1}}
	\tikzdef{\bicisizey}{\kv{s}*\kv{y}/20};
	\tikzdef{\bicisizex}{\biciflipper*\bicisizey};
	\coordinate(bicicentroruotaposteriore)at($(#2)+(-0.5*\biciflipper*\kv{s}*\kv{x}+3*\bicisizex,+3*\bicisizey)$);
	\coordinate(bicicentroruotaanteriore)at($(#2)+(+0.5*\biciflipper*\kv{s}*\kv{x}-3*\bicisizex,+3*\bicisizey)$);
